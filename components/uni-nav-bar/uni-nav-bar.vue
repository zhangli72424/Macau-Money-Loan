<template>
	<view class="uni-navbar" style="width: 100%;">
		<view class="uni-navbar__content" :class="{'uni-navbar--fixed': !!fixed,'uni-navbar--shadow':!!border,'uni-navbar--border':!!border}" :style="{'background-color':backgroundColor}" style="z-index: 9999;">
			<!-- 状态栏 -->
			<!-- #ifdef APP-PLUS -->
			<uni-status-bar v-if="statusBar" :bgcolor="bgcolor"></uni-status-bar>
			<!-- #endif -->
			<!-- 导航栏 -->
			<view class="uni-navbar__header" :style="{color:color}">
				<!-- 左边按钮 -->
				<view class="uni-navbar__header-btns" :class="{'uni-navbar__header-btnss':leftIcons}" @tap="onClickLeft" v-if="leftIcon.length">
					<view >
				...
				<!-- 右边按钮 -->
				<view class="uni-navbar__header-btns" @tap="onClickRight" v-if="rightText.length&&!rightIcon.length">
...
<script>
	...
	import uniIcon from '@/components/uni-icons/uni-icon.vue'
...
<style>
	...
	.uni-navbar__header-btns.uni-navbar__header-btnss{
		width: 60upx;
	}
